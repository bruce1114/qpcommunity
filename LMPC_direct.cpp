...
int main(){
    ...
    do{
        ...
        // tgraph.cntlayers=0;
        tgraph.validv.resize(tgraph.adjList.size());
        for(int i=0;i<tgraph.validv.size();++i) tgraph.validv[i]=true;
        sigma++;
    }while(true);
    gettimeofday(&aend,NULL);
    cerr<<sigma<<endl;
    cout<<(aend.tv_sec-astart.tv_sec)*1000+(aend.tv_usec-astart.tv_usec)/1000<<" "<<answer.size()<<endl;
